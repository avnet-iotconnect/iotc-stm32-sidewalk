...
v3: returns TWO top-level objects - {"payload": {...}, "ota": {...}}. Every OTA/FUOTA
field (all ota_* keys plus fw_version) is grouped under "ota"; everything else stays
under "payload". "ota" is always present (empty {} when the frame carries no OTA data).
...
def _parse_sid_demo(data):
    ...
    while tlv_off < len(payload):
        ...
        if   tag == SID_TAG_TEMPERATURE_SENSOR_DATA_NOTIFY and len(value) >= 2:
            ...
            # application version = major(2 BE) + minor(2 BE) + patch(2 BE)
            major = int.from_bytes(value[0:2], "big")
            minor = int.from_bytes(value[2:4], "big")
            patch = int.from_bytes(value[4:6], "big")
...
# ====== OTA/FUOTA field grouping ======================================

# Fields moved into the separate top-level "ota" object; everything else stays
# under "payload". Any key beginning with "ota" is also grouped here automatically.
OTA_FIELD_KEYS = (
    "ota_supported", "ota_trigger", "ota_percent", "ota_completed_bytes",
    "ota_total_bytes", "ota_status", "ota_status_code", "ota_file_id",
    "fw_version", "app_version",
)


def _split_ota(parsed):
    """Split a flat parsed dict into (payload_fields, ota_fields)."""
    payload, ota = {}, {}
    for k, v in parsed.items():
        if k.startswith("ota") or k in OTA_FIELD_KEYS:
            ota[k] = v
        else:
            payload[k] = v
    return payload, ota
...
def dict_from_payload(base64_input: str, fport: int = None):
    ...
    if 0x40 <= first <= 0x5F:
        ...
        parsed = _parse_sid_demo(data)
    else:
        # Anything else is treated as plain-TLV (Teseo standalone firmware).
        parsed = _parse_teseo(data)

    payload, ota = _split_ota(parsed)
    return {"payload": payload, "ota": ota}
...
if __name__ == "__main__":
    ...
    for b64, exp_pct, exp_comp, exp_tot in OTA_PROGRESS_SAMPLES:
        res = dict_from_payload(b64)
        ota = res["ota"]
        ok = (ota.get("ota_percent") == exp_pct
              and ota.get("ota_completed_bytes") == exp_comp
              and ota.get("ota_total_bytes") == exp_tot)
        all_ok = all_ok and ok
        print(f"  [{'PASS' if ok else 'FAIL'}] {b64}")
        print(f"         payload={json.dumps(res['payload'])}")
        print(f"         ota    ={json.dumps(ota)}")
        if not ok:
    ...
    comp_res = dict_from_payload(base64.b64encode(comp_raw).decode())
    print("\n=== FUOTA completion status (synthetic) ===")
    print(f"  payload={json.dumps(comp_res['payload'])}")
    print(f"  ota    ={json.dumps(comp_res['ota'])}")
    all_ok = all_ok and comp_res["ota"].get("ota_status") == "SUCCESS"

    # Capability heartbeat carrying SDK fw_version (0x0F) AND app_version (0x14):
    #   msg_desc 0x40, OTA_SUPPORTED=1, FW_VERSION 1.19.4-20, APP_VERSION 1.1.1, LINK_TYPE=1
    def _u16be(v):
        return bytes([(v >> 8) & 0xFF, v & 0xFF])
    cap_raw = (bytes([0x40, 0x0E, 0x01])
               + bytes([0xCF, 0x08]) + _u16be(1) + _u16be(19) + _u16be(4) + _u16be(20)
               + bytes([0xD4, 0x06]) + _u16be(1) + _u16be(1) + _u16be(1)
               + bytes([0x0C, 0x01]))
    cap_res = dict_from_payload(base64.b64encode(cap_raw).decode())
    print("\n=== Capability (fw_version + app_version) ===")
    print(f"  payload={json.dumps(cap_res['payload'])}")
    print(f"  ota    ={json.dumps(cap_res['ota'])}")
    all_ok = (all_ok
              and cap_res["ota"].get("fw_version") == "1.19.4-20"
              and cap_res["ota"].get("app_version") == "1.1.1")
